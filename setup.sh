...
mkdir -p src/{layers/{presentation,application,domain,infrastructure}/__tests__,core/{parser,validator,generator,runtime,security}/__tests__,modules/{users,posts,comments}/{domain,application/{commands,queries,read-models,write-models},infrastructure,events,dependencies,handlers,storage,ui,__tests__},events/__tests__,cqrs/__tests__,shared/{types,utils,constants},docs,monitoring}
...
for module in users posts comments; do
    ...
    touch src/modules/$module/application/__tests__/{commands,queries}/*.test.ts
